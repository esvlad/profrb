...
class ModelFaqView extends MVC{
	public function getContents($arg = array()){
		...
		foreach($result as $key => $value){
			...
			if(isset($value['answer'])) $value['answer'] = $this->typograf($value['answer']);
	...
	public function getContent($id){
		...
		$sql = 'SELECT fa.name as answer_name, fa.caption as answer_caption, c.title as category_title, f.* FROM '.DB_PREFIX.'faq f, '.DB_PREFIX.'faq_answer fa, '.DB_PREFIX.'category c  WHERE';
		$sql .= ' f.id = ?i AND fa.id = f.answer_author_id AND c.id = f.category_id AND f.active = ?i LIMIT ?i,?i';
	...
	public function getSearchContent($string){
		...
		WHERE f.id IN (?a) AND fa.id = f.answer_author_id AND c.id = f.category_id GROUP BY f.id ORDER BY f.date_creat DESC';
		...
		$comments = $this->db->getAll($sql_comments, $search_faq, 1);
		$com = array();
		foreach($comments as $comment){
			$com[$comment['faq_id']][] = $comment;
		}
