<h2 class="modal_title">Редактирование выплаты</h2>
...
<form class="admin_form admin_form_calc_cp clearfix" method="post" data-table="calc" data-params="update">
    ...
    <div class="admin_form_row">
      ...
      <select id="type" name="type">
        ...
        <? else : ?>
          <option value="1">Начисляется к оплате за фактический объем работы</option>
          <option value="2" selected>Начисляется на оклад (ставку) работника</option>
        <? endif; ?>
      </select>
    </div>
    <div class="admin_form_row">
    ...
    <div class="admin_form_row">
      <label for="oklad">Размер выплаты</label>
    ...
    <div class="admin_form_row">
      ...
      <select id="type_value" name="type_value">
        <? if($content['type_value'] == 1) : ?>
          <option value="1" selected>Коэфицент</option>
          <option value="2">Фактическая</option>
        <? else : ?>
          <option value="1">Коэфицент</option>
          <option value="2" selected>Фактическая</option>
        <? endif; ?>
      </select>
    </div>
        ...
        <? if(!empty($content['variants'])) : ?>
          ...
          <? foreach($variants as $variant) : ?>
            <div id="variant<?=$v;?>" class="admin_form_row">
              ...
              <input class="calc_var_value" type="text" name="variants[<?=$v;?>][value]" data-table="calc_jobs" value="<?=$variant->value;?>" placeholder="Размер выплаты" />
            </div>
            <? $v++; ?>
          <? endforeach; ?>
        <? endif; ?>
...
$('.admin_modal #edit').click(function(e){
  ...
  if(!form_error){
    ...
    $.ajax({
      url: 'index.php?r=admin/calculator/updatePays',
      type: 'post',
      data: post_data,
      dataType: 'json',
