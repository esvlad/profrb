...
<form class="admin_form clearfix" method="post" action="index.php?r=admin/content/save_update&type=static" data-table="content" data-params="save">
    <input type="hidden" name="content_type_id" value="7">
      ...
      <? if(!empty($fields)) : ?>
        <? foreach($fields as $key => $field) : ?>
          <? if($field['name'] == 'workers_image') : ?>
            <div class="admin_form_row">
              <label for="workers_image">Фото сотрудника</label>
              <div id="fileUpload<?=$key;?>" class="file_upload clearfix">
                <input id="workers_image" type="file" name="workers_image[]" data-table="fields" data-file-type="images" class="file_upload_input" onchange="getUploadFiles('workers_image');" value="">
                <?=HtmlHelper::crutchLaborImage($field);?>
                <div id="fileUploaded" class="btn btn_admin" onclick="fileUploadBtn('workers_image');">Добавить файл</div>
                <input class="file_upload_filed" type="hidden" name="workers_image[<?=$field['id'];?>]" value="">
              </div>
              <p class="admin_form_caption">Можно прикрепить лишь 1 фотографию. Разрешены форматы: JPG, PNG, GIF. Предпочтительные параметры фото 306 x 330 пикселей (Ш x В).</p>
            </div>
          <? else : ?>
